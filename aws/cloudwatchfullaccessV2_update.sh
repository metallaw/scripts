...
NEW_POLICY_ARN="arn:aws:iam::aws:policy/CloudWatchFullAccessV2"
...
if [ ${#IAM_ROLES[@]} -gt 0 ]; then
    for i in ${!IAM_ROLES[@]}
        do
            aws iam attach-role-policy --role-name ${IAM_ROLES[$i]} --policy-arn ${NEW_POLICY_ARN}
            echo ${IAM_ROLES[$i]}
...
if [ ${#IAM_USERS[@]} -gt 0 ]; then
    for i in ${!IAM_USERS[@]}
        do
            aws iam attach-user-policy --user-name ${IAM_USERS[$i]} --policy-arn ${NEW_POLICY_ARN}
            echo ${IAM_USERS[$i]}
...
if [ ${#IAM_GROUPS[@]} -gt 0 ]; then
    for i in ${!IAM_GROUPS[@]}
        do
            aws iam attach-group-policy --group-name ${IAM_GROUPS[$i]} --policy-arn ${NEW_POLICY_ARN}
            echo ${IAM_GROUPS[$i]}
